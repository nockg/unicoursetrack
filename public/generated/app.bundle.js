...
async function signUpFromModal() {
  ...
  if (!email || !password || !confirmPassword) {
    setAuthError("Enter an email, password, and confirmation password.");
    return;
  }
...
async function loadCloudSave() {
  ...
  let error;
  try {
    ({ data, error } = await withCloudTimeout(supabaseClient
      .from("tracker_profiles")
      .select("data, prefs, updated_at")
      .eq("user_id", currentUser.id)
      .maybeSingle(), "Cloud data load"));
  } catch (cloudError) {
  ...
  if (data?.prefs) {
    Object.assign(preferences, data.prefs);
    localStorage.setItem(PREFS_KEY, JSON.stringify(preferences));
  }
...
supabaseClient.auth.onAuthStateChange(async (event, session) => {
  ...
  setTimeout(() => setAuthError("Cloud sign-in did not load. Refresh the page and check your internet connection."), 0);
}
...
async function clearTrackerStorage() {
  ...
  if (currentUser) {
    const { error } = await supabaseClient
      .from("tracker_profiles")
      .upsert({ user_id: currentUser.id, data: blankState, prefs: blankPrefs, updated_at: new Date().toISOString() }, { onConflict: "user_id" });
    if (error) {
      await showAppNotice("Could not clear cloud storage", error.message);
      cloudReady = true;
      return;
    }
  }
