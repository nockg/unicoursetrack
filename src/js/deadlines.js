...
import { getStore, save, syncModalScrollLock, updateCalendarComposerMode } from './state.js';
import { escapeHtml, toDateInputValue, toTimeInputValue, navigateCalendarWindow, openTrustedUrl } from './utils.js';
import { formatCountdown } from './dashboard.js';
...
function deadlineModuleLabel(deadline) {
  if (Number.isInteger(deadline?.moduleIndex) && store.MODULES[deadline.moduleIndex]) {
    return store.MODULES[deadline.moduleIndex].kanji || store.MODULES[deadline.moduleIndex].short || store.MODULES[deadline.moduleIndex].name;
  }
  return 'General';
}
...
function renderDeadlineModuleOptions(value = '') {
  ...
  select.innerHTML = `<option value="">General</option>` +
    store.MODULES.map((mod, mi) => `<option value="${mi}">${escapeHtml(mod.kanji || mod.short || mod.name)}</option>`).join('');
...
export function setCalendarComposerPriority(priority = 'default') {
  const selected = DEADLINE_PRIORITY_COLOURS[priority] ? priority : 'default';
...
export function updateDeadlineFormMode() {
...
export function showDeadlineTab(tab = 'upcoming') {
...
export function renderStickyExams() {
  const host = document.getElementById('live-exam-bar');
  if (!host) return;
  const ys = getStore();
  const now = Date.now();
  const upcoming = (ys.customExams || [])
  ...
  upcoming.forEach((exam) => {
    ...
    `;
    card.addEventListener('contextmenu', (event) => { event.preventDefault(); window.removeExam?.(exam.originalIndex); });
...
// ── Deadline form open/close ───────────────────────────────────────────────

export function openDeadlineForm(index = null, type = 'date') {
...
function buildDeadlineFromForm() {
  ...
  if (!mod) { alert('Please enter a deadline title.'); return null; }
  if (!date) { alert('Please choose a start date.'); return null; }

  if (activeDeadlineFormType !== 'event') {
    const parsed = new Date(`${date}T${time || '09:00'}`);
    if (Number.isNaN(parsed.getTime())) { alert('Please enter a valid date and time.'); return null; }
    return {
      type: 'date', mod,
      ...
      allDay: false, availability: 'BUSY', location: '',
  ...
  if (Number.isNaN(parsed.getTime())) { alert('Please enter a valid event start date and time.'); return null; }
...
export function saveDeadlineForm(openCalendar = false) {
  ...
  const ys = getStore();
  if (editingDeadlineIndex !== null && ys.customExams[editingDeadlineIndex]) {
    deadlineData.completed = !!ys.customExams[editingDeadlineIndex].completed;
    ys.customExams[editingDeadlineIndex] = deadlineData;
  } else {
    ys.customExams.push(deadlineData);
  }
  save();
  renderStickyExams();
  closeDeadlineForm();
...
function downloadCalendarIcs(eventData) {
  ...
  const content = [
    'BEGIN:VCALENDAR', 'VERSION:2.0', 'PRODID:-//UniTrack//Deadline Export//EN', 'BEGIN:VEVENT',
    ...
    `SUMMARY:${safeTitle}`, `DESCRIPTION:${details}`,
...
function openCalendarEvent(eventData) {
  ...
  if (providerKey === 'apple') { downloadCalendarIcs(eventData); return; }
  if (providerKey === 'outlook') {
    const params = new URLSearchParams({
      subject: eventData.title, startdt: eventData.start.toISOString(), enddt: eventData.end.toISOString(),
      body: eventData.details || '', location: eventData.location || '',
      allday: eventData.allDay ? 'true' : 'false', ctz: timezone,
    });
  ...
  const params = new URLSearchParams({
    ...
    details: eventData.details || '', location: eventData.location || '',
    crm: eventData.availability || 'BUSY', ctz: timezone,
  });
...
function openDeadlineInCalendar(deadline) {
  ...
  openCalendarEvent({
    title: deadline.mod, start, end, allDay: !!deadline.allDay,
...
export function openDeadlineInCalendarByIndex(index, event) {
...
// ── Deadline bar scrolling ─────────────────────────────────────────────────

export function setupDeadlineBarScrolling() {
  const bar = document.getElementById('live-exam-bar');
  if (!bar || bar.dataset.scrollReady) return;
  bar.dataset.scrollReady = 'true';
  let isDown = false, startX = 0, startScrollLeft = 0, moved = false, pressedDeadlineIndex = null;
  ...
  bar.addEventListener('pointerdown', (event) => {
    ...
    startX = event.clientX; startScrollLeft = bar.scrollLeft;
  ...
  function endDrag(event) {
    if (!isDown) return;
    const releasedCard = document.elementFromPoint(event.clientX, event.clientY)?.closest?.('.exam-ticker') || event.target.closest('.exam-ticker');
    ...
    if (shouldOpen) { const index = Number(releasedDeadlineIndex); if (Number.isFinite(index)) openDeadlineForm(index); }
  ...
  bar.addEventListener('click', (event) => {
    if (!moved) {
      if (event.target.closest('.exam-ticker')) { event.preventDefault(); event.stopPropagation(); }
      return;
    }
...
export function renderDeadlineTimeline(force = false) {
  const host = document.getElementById('timeline-list');
  if (!host) return;
  if (activeDeadlineTab === 'add' && !force) { host.innerHTML = renderDeadlineAddChoice(); return; }
  const ys = getStore();
  const now = Date.now();
  const exams = (ys.customExams || [])
    .map((exam, originalIndex) => ({ ...exam, originalIndex }))
    .filter((exam) => !exam.completed)
    .sort((a, b) => new Date(a.date) - new Date(b.date));
...
export function updateDeadlineNote(index, value) {
  const ys = getStore();
  if (!ys.customExams[index]) return;
  ys.customExams[index].note = value;
  save();
}

export function toggleDeadlineComplete(index, event) {
  if (event) event.stopPropagation();
  const ys = getStore();
  if (!ys.customExams[index]) return;
  ...
  if (row) { row.classList.add('completing'); setTimeout(removeDeadline, 280); return; }
  removeDeadline();
}

export function openDeadlineTimeline() {
...
export function updateDeadlineCountdowns() {
  document.querySelectorAll('[data-deadline-countdown]').forEach((node) => {
    const date = node.dataset.deadlineCountdown;
    node.textContent = node.classList.contains('deadline-card-countdown')
      ? `Due in ${formatCountdown(date)}`
      : formatCountdown(date);
    node.classList.toggle('urgent', new Date(date).getTime() <= Date.now());
  });
}

export function closeDeadlineTimeline() {
  document.getElementById('timeline-modal').classList.add('hidden');
  syncModalScrollLock();
  if (deadlineTimelineInterval) { clearInterval(deadlineTimelineInterval); deadlineTimelineInterval = null; }
}
